(function() {
    ...
    wmu.extend(Markers.prototype, {
        ...
        addLine: function(line, options) {
            for (var i = 0; i < line._points.length; ++i) {
                var point = line._points[i];
                if (!this._clusterRoot._points[point._id]) {
                    if (options && options.addPoints === true) {
                        this._clusterRoot.addPoint(point);
                    } else {
                        throw "Line contains points not previously added. Add points before calling addLine, or set options.addPoints parameter to true."
                    }
                }
            }
