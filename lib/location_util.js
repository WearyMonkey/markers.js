(function() {
    ...
    $.extend(Markers.prototype, {
        ...
        setClusterState: function(cluster, state) {
            ...
            if (state == Cluster.States.Collapsed) {
                zoomOut(this, root, this.googleMap.getZoom(), root);
                root.setState(newState, true);
            } else if (state == Cluster.States.Expanded) {
                root.setState(newState, true);
                zoomIn(this, root, this.googleMap.getZoom(), root);
            }
        }
    });

    function showCluster(self, cluster, center) {
        if (!self.createMarkerFn) return null;
        cluster.keepKey = self.keepKey;
        self.visibleClusters.push(cluster);

        if (!cluster.marker) {
            cluster.marker = self.createMarkerFn(cluster);
        }
        ...
        if (!cluster.marker.getMap()) {
            cluster.marker.setMap(self.googleMap);
    ...
    function showConnection(self, connection) {
        if (!self.createPolylineFn) return null;
        connection.keepKey = self.keepKey;
        self.visibleConnections.push(connection);

        if (!connection.polyline) {
            connection.polyline = self.createPolylineFn(connection.cluster1, connection.cluster2);
        }
        ...
        if (!connection.polyline.getMap()) {
            connection.polyline.setMap(self.googleMap);
    ...
    function move(self, root) {
        var i;
        var visible = root.getContainedClustersAndConnections(getSearchBounds(self), self.googleMap.getZoom());
    ...
    function zoomIn(self, root, zoom, overrideParent) {
        ...
        var childMarkers = [];
        var mapZoom = self.googleMap.getZoom();
        var self = self;
        ...
        var childrenToAnimate = getChildrenToAnimate(self, visible, root, addChild);
    ...
    function zoomOut(self, root, zoom, overrideParent) {
        ...
        var animatedChildren = getChildrenToAnimate(self, visible, root, addChild);
    ...
    function getChildrenToAnimate(self, visible, root, addChildfn) {
    ...
    function resetViewport(self) {
        ...
        var zoomShift = self.googleMap.getZoom() - self.prevZoom;
        if (zoomShift == 1) {
            zoomIn(self, self.clusterRoot, self.googleMap.getZoom());
        } else if (zoomShift == -1) {
            zoomOut(self, self.clusterRoot, self.googleMap.getZoom()+1);
        } else {
