...
$.extend(Markers.prototype, {
    ...
    showCluster: function(cluster, center) {
        ...
        if (!cluster.marker) {
            cluster.marker = this.createMarkerFn(cluster);
        }
    ...
    showConnection: function(connection) {
        ...
        if (!connection.polyline) {
            connection.polyline = this.createPolylineFn(connection.cluster1, connection.cluster2);
        }
    ...
    move: function (root) {
        var self = this, i;
        var visible = root.getContainedClustersAndConnections(getSearchBounds(this), this.googleMap.getZoom());
    ...
    zoomIn: function(root, zoom, overrideParent) {
        var visible = root.getContainedClustersAndConnections(getSearchBounds(this), zoom);
        var childMarkers = [];
